...
export const scope = {
	// invoker,
	[BackgroundColorCommand.name]: (color: string) => invoker
		.executeCommand(new BackgroundColorCommand(color)),
	[CursorCommand.name]: (cursor: string) => invoker
		.executeCommand(new CursorCommand(cursor)),
};
